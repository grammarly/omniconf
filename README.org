...
(cfg/define
  {:hostname {:description "where service is deployed"
   ...
   2. Populate the configuration from available sources:
      ...
      #+BEGIN_SRC clojure
      (cfg/populate-from-cmd args) ;; args is a CLI options list
      ...
      make environment variables overwrite CLI options, or give the highest priority to the config file
      In the above example we get the path to the
      configuration file as =--conf= CLI option. For more information,
      see <TODO>.
...
** Configuration scheme syntax
   ...
   Configuration scheme is a map of option names to the maps of their parameters.
...
*** Providing configuration as files
    ...
    the CLI options. So you have to =populate-from-cmd= first, and then to
    ...
    always desirable. As a workaround, you can call =populate-from-cmd= again,
    but only if your CLI args are idempotent (i.e. do not contain =^:concat=).
...
(cfg/set-logging-fn (fn [& args]
    ...
    Note that it will only work if you are able to initialize logging without
    any data from Omniconf. This is a chicken-and-egg problem that doesn't have a
