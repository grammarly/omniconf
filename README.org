...
(cfg/define
  {:hostname {:description "where service is deployed"
   ...
   2. Populate the configuration from available sources:
      ...
      deny environment variables overwrite command-line args, or make the config
      file have the highest priority. In the above example we get the path to
      the configuration file as =--conf= CMD argument. For more information on
      that see [[https://github.com/grammarly/omniconf#providing-configuration-as-files][this]].
...
** Configuration scheme syntax
   ...
   Option name is a keyword that will denotes how the option is retrieved inside
...
*** Providing configuration as files
    ...
    again, but only if your CLI args are idempotent (do not contain =^:concat=).
